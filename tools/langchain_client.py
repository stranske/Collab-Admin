...
def build_chat_client(
    *,
    ...
    if selected_provider == PROVIDER_ANTHROPIC:
        if not anthropic_token or not chat_anthropic:
            return None
        try:
    ...
    for slot in slots:
        ...
        if slot.provider == PROVIDER_ANTHROPIC and anthropic_token and chat_anthropic:
            with contextlib.suppress(Exception):
                client = _build_anthropic_client(
                    chat_anthropic,
                    model=slot_model,
...
def build_chat_clients(
    *,
    ...
    if selected_provider:
        ...
        elif selected_provider == PROVIDER_ANTHROPIC and anthropic_token and chat_anthropic:
            with contextlib.suppress(Exception):
                clients.append(
                    ClientInfo(
                        client=_build_anthropic_client(
    ...
    for slot in slots:
        if any(
            (
                ...
                slot.provider == PROVIDER_ANTHROPIC and anthropic_token and chat_anthropic,
